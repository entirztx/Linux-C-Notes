...
void pthread_testcancel(void);	//本函数什么都不做，本身就是一个取消点
```
...
int pthread_detach(pthread_t thread);	//分离一个线程
										//已经被detach的线程是不能被pthread_join()收回来
```
...
pthread_mutex_t fastmutex = PTHREAD_MUTEX_INITIALIZER;	//静态初始化（使用默认属性）
...
int  pthread_once(pthread_once_t *once_control, void (*init_routine)(void));	//保证被调用的函数只会执行一次
```
